...
public class SaleController {
    ...
    //promedios de ventas por vendedores
    ...
    public ResponseEntity<ResponseDTO> getAverageSaleByStore() {
        ...
        while (findSale != 0) {
            return new ResponseEntity<>(new ResponseDTO(200, saleService.getTotalSales() / (float) storeService.getStores().size(), null),
                    HttpStatus.OK);

        }
        return new ResponseEntity<>(new ResponseDTO(409, "no hay ventas , no se puede obtener un promedio ", null),
                HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<ResponseDTO> getAverageSaleBySellers() {
        int findSale = saleService.getTotalSales();
        while (findSale != 0) {
            return new ResponseEntity<>(new ResponseDTO(200, saleService.getTotalSales() / (float) sellerService.getSellers().size(), null),
                    HttpStatus.OK);
